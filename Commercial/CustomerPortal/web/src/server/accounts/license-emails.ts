...
export async function sendLicenseCreatedEmail(input: {
  ...
  const subject = `Your license key — ${pkg} · THE GOLD MIND PROFESSIONAL`;
  const text = [
    `Hello ${name},`,
    ``,
    `Your THE GOLD MIND PROFESSIONAL license is ready.`,
    ``,
    `Account: ${input.to}`,
    `Package: ${pkg}`,
  ...
  const html = `
    ...
    <p>Your <strong>THE GOLD MIND PROFESSIONAL</strong> license is ready.</p>
    <ul>
...
export async function sendLicenseActivatedEmail(input: {
  ...
  const pkg = packageLabel(input.packageType);
  const name = input.customerName?.trim() || "Customer";
  const subject = `License activated — ${pkg} · THE GOLD MIND PROFESSIONAL`;
  const text = [
    `Hello ${name},`,
    ``,
    `Your THE GOLD MIND PROFESSIONAL license is now active.`,
    ``,
    `Account: ${input.to}`,
    `Package: ${pkg}`,
  ...
  const html = `
    <p>Hello <strong>${escapeHtml(name)}</strong>,</p>
    <p>Your <strong>THE GOLD MIND PROFESSIONAL</strong> license is now <strong>active</strong>.</p>
    <ul>
      <li><strong>Account:</strong> ${escapeHtml(input.to)}</li>
      <li><strong>Package:</strong> ${escapeHtml(pkg)}</li>
